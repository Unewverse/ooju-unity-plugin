...
namespace OojiCustomPlugin
{
    public static class OOJUSceneImportUtility
    {
        public static async Task<GameObject?> ImportSceneZipAsync(string zipPath)
        {
            ...
            var json      = File.ReadAllText(Path.Combine(tempDir, "scene.json"));
            ...
            foreach (var ob in sceneFile.objects)
            {
                ...
                if (ob.type == "mesh" && !string.IsNullOrEmpty(ob.mesh))
                {
                    ...
                    var dstPath = Path.Combine(dstDir, Path.GetFileName(glbPath));
                    File.Copy(glbPath, dstPath, true);
                    AssetDatabase.Refresh();
                    var prefab = AssetDatabase.LoadAssetAtPath<GameObject>(dstPath);
                    if (prefab) PrefabUtility.InstantiatePrefab(prefab, go.transform);
#endif
                }
        ...
        [System.Serializable] class Xf         { public float[] position; public float[] rotation; public float[] scale; }
        [System.Serializable] class Primitive  { public string kind; public float[] size; public float radius; public float height; }
    }
}
